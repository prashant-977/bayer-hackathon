...
class VisualizationGenerator:
    ...
    def extract_insights(self, df: pd.DataFrame, context: Dict) -> Dict:
        ...
        if context["temporal_cols"]:
            ...
            if len(df_dates) > 0:
                insights["temporal"] = {
                    ...
                    "observations_per_day": len(df_dates)
                    / max((df_dates.max() - df_dates.min()).days, 1),
                }
        ...
        if len(context["temporal_cols"]) >= 2:
            ...
            durations = durations.dt.total_seconds() / (24 * 3600)
            ...
            if len(durations) > 0:
                insights["handling_time"] = {
                    ...
                    "max": durations.max(),
                    "std": durations.std(),
                    "within_1_day": int((durations <= 1).sum()),
                    "within_3_days": int((durations <= 3).sum()),
                    "over_5_days": int((durations > 5).sum()),
                }
            ...
            if not value_counts.empty:
                insights["distribution"] = {
                    ...
                    "most_common_percentage": round(
                        (value_counts.values[0] / len(df)) * 100, 1
                    ),
        ...
        for col in df.columns:
            ...
            if ptypes.is_object_dtype(dtype) or ptypes.is_string_dtype(dtype):
                ...
                if avg_length > 30 and unique_ratio > 0.3:
                    ...
                    # Default to categorical for short strings with many repeats
                    if avg_length < 30 and unique_ratio < 0.8:
                        categorical_cols.append(col)
                    else:
                        text_cols.append(col)
    ...
    def select_visualization_type(self, context: Dict) -> str:
        ...
        prompt_lower = context.get("prompt_lower", "")

        # DURATION/TIME questions → Histogram
        if "duration" in intents or any(
        ...
        if "temporal" in intents or any(
            term in prompt_lower for term in ["trend", "trendi", "over time", "change", "muutos", "kehitys"]
        ):
            if context["temporal_cols"]:
                return "time_series"

        # STATUS/DISTRIBUTION questions → Bar chart
        if "status" in intents or "distribution" in intents or any(
            term in prompt_lower for term in ["status", "tila", "distribution", "jakauma", "breakdown"]
        ):
            if context["categorical_cols"] or context["text_cols"]:
                return "bar_chart"

        # COUNT/TOP questions → Bar chart
        if "count" in intents or "top" in intents or any(
            term in prompt_lower for term in ["how many", "montako", "most common", "yleisimmät", "eniten"]
        ):
            if context["categorical_cols"] or context["text_cols"]:
                return "bar_chart"
    ...
    def prepare_data(self, df: pd.DataFrame, viz_type: str, context: Dict) -> Optional[pd.DataFrame]:
        ...
        for col in context.get("temporal_cols", []):
            if not ptypes.is_datetime64_any_dtype(df_copy[col].dtype):
                df_copy[col] = pd.to_datetime(df_copy[col], errors="coerce")

        # Universal: Calculate duration if 2+ date columns
        if len(context.get("temporal_cols", [])) >= 2:
            start_col = context["temporal_cols"][0]
            end_col = context["temporal_cols"][1]
            df_copy["calculated_duration_days"] = (
                pd.to_datetime(df_copy[end_col], errors="coerce")
                - pd.to_datetime(df_copy[start_col], errors="coerce")
            ).dt.total_seconds() / (24 * 3600)

            # Remove negative durations and NaNs
            df_copy = df_copy[df_copy["calculated_duration_days"].notna()]
    ...
    def _prepare_histogram_data(self, df: pd.DataFrame, context: Dict) -> Optional[pd.DataFrame]:
        ...
        elif context.get("numerical_cols"):
    ...
    def _prepare_timeseries_data(self, df: pd.DataFrame, context: Dict) -> Optional[pd.DataFrame]:
        ...
        if not context.get("temporal_cols"):
            return None
    ...
    def _prepare_categorical_data(self, df: pd.DataFrame, context: Dict) -> Optional[pd.DataFrame]:
        ...
        if context.get("categorical_cols"):
            cat_col = context["categorical_cols"][0]
            counts = df[cat_col].fillna("Missing").astype(str).value_counts().reset_index()
            counts.columns = ["category", "count"]
            counts = counts.head(self.viz_config["max_categories"])
            return counts

        # Extract from text
        if context.get("text_cols"):
    ...
    def _extract_by_tfidf(self, df: pd.DataFrame, context: Dict) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            for i in range(len(texts)):
                ...
                top_idx = int(doc_scores.argmax())
                top_word = feature_names[top_idx]
                category = top_word.title()
                categories.append(category)

            category_counts = Counter(categories)
            result = pd.DataFrame(list(category_counts.items()), columns=["category", "count"]).sort_values(
    ...
    def _create_histogram(self, data: pd.DataFrame, context: Dict) -> go.Figure:
        ...
        if handling_time:
            avg_val = handling_time.get("average", float(data[num_col].mean()))
            median_val = handling_time.get("median", float(data[num_col].median()))
            ...
            stats_text = (
                ...
                f"Total: {insights.get('total_count', len(data))} observations<br>"
                f"Average: {avg_val:.1f} days<br>"
                f"Median: {median_val:.1f} days<br>"
                f"Range: {handling_time.get('min', 0):.1f} - {handling_time.get('max', 0):.1f} days"
            )
    ...
    def _create_time_series(self, data: pd.DataFrame, context: Dict) -> go.Figure:
        ...
        fig.update_traces(line_color="#1f77b4", line_width=2, marker=dict(size=8))

        # Add insights
        insights = context.get("insights", {})
        temporal_info = insights.get("temporal", {})

        if temporal_info and not data.empty:
            ...
            peak_date = data.loc[peak_idx, "date"]
            peak_count = int(data.loc[peak_idx, "count"])
            ...
            summary_text = (
                f"<b>Summary:</b><br>"
                f"Total: {total} observations<br>"
                f"Period: {span_days} days<br>"
                f"Average: {avg_per_day:.1f} per day<br>"
    ...
    def _create_bar_chart(self, data: pd.DataFrame, context: Dict) -> go.Figure:
        ...
        fig = px.bar(
            ...
            labels={x_col: x_col.replace("_", " ").title(), y_col: "Number of Observations"},
            color=y_col,
        ...
        total = insights.get("total_count", int(data_sorted[y_col].sum()))
        ...
        if distribution:
            ...
            summary_text = (
                f"<b>Summary:</b><br>"
                f"Total: {total} observations<br>"
                f"Most common: {most_common}<br>"
                f"({most_common_pct:.1f}% of total)"
            )
        else:
            summary_text = f"<b>Total:</b> {total} observations<br>Showing top {len(data_sorted)} categories"
